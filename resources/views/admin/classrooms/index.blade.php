...
<body class="bg-gray-100 min-h-screen">
    <header class="bg-indigo-600 text-white shadow-md">
        <div class="max-w-7xl mx-auto p-4 flex justify-between items-center">
            ...
            <a href="{{ url('/classrooms/create') }}" class="bg-white text-indigo-600 px-4 py-2 rounded-lg font-semibold hover:bg-indigo-100 transition">
                + Tambah Kelas
            </a>
        </div>
    </header>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-indigo-50 border-b">
                                ...
                                <th class="py-3 px-4 font-semibold text-gray-700">Deskripsi</th>
                        ...
                        <tbody>
                            @foreach ($classrooms as $i => $classroom)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="py-3 px-4 font-medium text-gray-800">{{ $classroom->name ?? '-' }}</td>
                                    <td class="py-3 px-4">{{ $classroom->capacity ?? '—' }}</td>
                                    <td class="py-3 px-4 text-gray-600">{{ $classroom->description ?? 'Tidak ada deskripsi' }}</td>
